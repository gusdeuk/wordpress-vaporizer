...
function emm_paginate($args = null) {
	$defaults = array(
		...
		'before' => '<div class="pagination-wrapper"><ul class="pagination">', 'after' => '</ul></div>',
		//'nextpage' => __('&raquo;'), 'previouspage' => __('&laquo'),
		'nextpage' => "<span class = 'fa fa-arrow-right'></span>",
		'previouspage' => "<span class = 'fa fa-arrow-left'></span>",
		'echo' => 1
	);
  ...
  $r = wp_parse_args($args, $defaults);
	...
	if ($pages > 1) {   
		...
		if ($page > 1 && !empty($previouspage)) {
			...
			$output .= "<li><a class='disabled' href='javascript:void();'>$previouspage</a></li>";
		}
		...
		if ($page < $pages && !empty($nextpage)) {
			$output .= "<li><a href='" . get_pagenum_link($page + 1) . "'>$nextpage</a></li>";
			 ...
			 $output .= "<li><a  class='disabled' href='javascript:void();'" . get_pagenum_link($page + 1) . "'>$nextpage</a></li>";
		}
